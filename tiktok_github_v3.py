...
def load_sched():
    ...
    for i in range(1, 11):
        it = by_rank[i]
        ln = [l for l in nar if l["key"] == f"repo{i}"][0]
...
def cap_words(it, text, gain_kw):
    ...
    for w_ in text.split():
        c = V.RED if (gain_kw in w_ or w_ in ("Quán", "quân", "mạnh", "mẽ", "top", "mắt", "chú", "chốt", "Lưu", "lại")) else V.WHITE
        words.append((w_, c))
    return words
...
def draw_roll_v3(wide, it, ln, ch, elapsed, dur):
    ...
    V.draw_karaoke(d, [words], V.F_CAP, W / 2, 950, min(0.99, (elapsed / ln["dur"]) / 2.4) if ln["dur"] else 0.99)
    return out
...
def main():
    ...
    with open(lst, "w", encoding="utf-8") as f:
        for kind, pay, dur, ch in segs:
            if kind == "hook":
                ...
                if dur - ln["dur"] > 0.05:
                    s = silence(dur - ln["dur"], f"hook{idx}"); f.write(f"file '{s}'\n"); parts.append(s)
            elif kind == "card":
                s = silence(dur, f"card{idx}"); f.write(f"file '{s}'\n"); parts.append(s)
            elif kind == "roll":
                it, ln, _ = pay
                if ln.get("mp3"):
                    f.write(f"file '{ln['mp3']}'\n"); parts.append(ln["mp3"])
                if dur - ln["dur"] > 0.05:
                    s = silence(dur - ln["dur"], f"roll{idx}"); f.write(f"file '{s}'\n"); parts.append(s)
            else:
                ln, _ = pay
                f.write(f"file '{ln['mp3']}'\n"); parts.append(ln["mp3"])
                if dur - ln["dur"] > 0.05:
                    s = silence(dur - ln["dur"], f"cta{idx}"); f.write(f"file '{s}'\n"); parts.append(s)
            idx += 1
    narr = os.path.join(TMP, "narration_full.mp3")
    subprocess.run([FF, "-y", "-f", "concat", "-safe", "0", "-i", lst, "-c", "copy", narr],
                   capture_output=True)
    final = os.path.join(BASE, "tiktok-github-v3.mp4")
    subprocess.run([FF, "-y", "-i", silent, "-i", narr, "-c:v", "copy", "-c:a", "aac",
                    "-shortest", final], capture_output=True)
    print(f"Saved: {final} ({os.path.getsize(final) // 1024} KB)")
    cap = ("Top 10 GitHub tuần này — bản có thuyết minh tiếng Việt! (số liệu thật) "
    ...
    open(os.path.join(BASE, "tiktok-github-v3-caption.txt"), "w", encoding="utf-8").write(cap + "\n")
    for f_ in ("tiktok-github-v3.mp4", "tiktok-github-v3-cover.png", "tiktok-github-v3-caption.txt"):
